...
test("Deve criar um pedido com 3 itens com um cupom de desconto expirado", function () {
    const cpf = "839.435.452-10"
    const order = new Order(cpf, new Date("2020-10-11"))
    order.addItem(new Item(1, "Música", "CD", 30), 3)
    order.addItem(new Item(1, "Video", "DVD", 50), 1)
    order.addItem(new Item(1, "vídeo", "VHS", 10), 2)
...
test("Deve criar um pedido com 3 itens com o cálculo do frete com a estrategia default", function () {
    const cpf = '839.435.452-10'
    ...
    order.addItem(new Item(6, 'Accessorios', 'Cabo', 30, 10, 10, 10, 0.9), 3)
    const freight = order.getFreight()
    expect(freight).toBe(260)
})

test("Deve criar um pedido com 3 itens com o cálculo do frete com a estrategia fixo", function () {
    const cpf = '839.435.452-10'
...
test("Deve criar um pedido com código", function () {
    ...
    expect(code).toBe(`${new Date().getFullYear()}00000001`)
})